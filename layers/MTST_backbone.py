...
class TSTEncoderLayer(nn.Module):
    def __init__(self, q_len, d_model, n_heads, d_k=None, d_v=None, d_ff=256, store_attn=False,
        ...
        self.cross_attn =_MultiheadAttention(d_model, n_heads, d_k, d_v, attn_dropout=attn_dropout, proj_dropout=dropout, res_attention=res_attention, cfg=cfg)
        # Add & Norm
...
#my
        # q = src[:, -1:, :]  # [bs, 1, patch_num, d_model]
        # # Extract key (K) and value (V) from the rest of the variables and the previous time step
        # k = torch.cat([src[:, :-1, :], prev[:, :-1, :]], dim=2) if prev is not None else src[:, :-1, :]  # [bs, nvar-1, 2*patch_num, d_model]
        # v = k.clone()  # [bs, nvar-1, 2*patch_num, d_model]
        #
        # # Cross-Attention
        # if self.res_attention:
        #     q, cross_attn, cross_scores = self.cross_attn(q, k, v, key_padding_mask=key_padding_mask, attn_mask=attn_mask,
        #                                                  attn_bias=attn_bias)
        # else:
        #     q, cross_attn = self.cross_attn(q, k, v, key_padding_mask=key_padding_mask, attn_mask=attn_mask,
        #                                     attn_bias=attn_bias)
        #     # Combine the output of cross-attention with the rest of the variables
        #     src = torch.cat([src[:, :-1, :], q], dim=1)  # [bs, nvar, patch_num, d_model]
        #
        # ## Add & Norm
