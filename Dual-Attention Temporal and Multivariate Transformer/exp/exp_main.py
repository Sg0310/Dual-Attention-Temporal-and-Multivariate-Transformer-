    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                preds_notinverse.append(outputs)
                trues_notinverse.append(batch_y)

                if test_data.scale and self.args.inverse:
                    shape = outputs.shape
                    outputs1 = test_data.inverse_transform(outputs.reshape(shape[0] * shape[1], -1)).reshape(shape)
                    batch_y1 = test_data.inverse_transform(batch_y.reshape(shape[0] * shape[1], -1)).reshape(shape)

                pred = outputs1  # outputs.detach().cpu().numpy()  # .squeeze()
                true = batch_y1  # batch_y.detach().cpu().numpy()  # .squeeze()
        ...
        preds1 = np.concatenate(preds_notinverse, axis=0)
        trues1 = np.concatenate(trues_notinverse, axis=0)
        inputx1 = np.concatenate(inputx, axis=0)
        preds1 = rearrange(preds1, 'b l d -> b d l')
        trues1 = rearrange(trues1, 'b l d -> b d l')
        inputx = rearrange(inputx1, 'b l d -> b d l')
        ...
        mae, mse, rmse, mape, mspe, rse, corr = metric(preds1, trues1)
        ...
        np.save(folder_path + 'metrics.npy', np.array([mae, mse, rmse, mape, mspe, rse, corr], dtype=object))
        np.save(folder_path + 'pred.npy', preds2)
        np.save(folder_path + 'true.npy', trues2)
